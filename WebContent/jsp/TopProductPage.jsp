...
<div class="container">
    ...
    <%
		...
		ArrayList<Detail> list =detailDAO.getList(boardID);
		for(int i =0; i<list.size(); i++){
	%>
        <div class="col-md-3 col-sm-6">
            <div class="product-grid2">
                <div class="product-image2">
                    <a href="DetailPageView.jsp?productCode=<%= list.get(i).getProductCode()%>">
                        <% 	
						String real = "C:\\Users\\leedh\\JSP\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\LDHPortFolio_Board\\img1";
						File viewFile = new File(real+"\\"+list.get(i).getProductCode()+"상세페이지용사진.jpg");
						if(viewFile.exists()){
						%> 
                       	<img class="pic-1" style="height: 200px" src="../img1/<%= list.get(i).getProductCode()%>상세페이지용사진.jpg">
                    	<%
						}
                    	%>
                    </a>
                ...
                <div class="product-content">
                    <h3 class="title"><a href="DetailPageView.jsp?productCode=<%= list.get(i).getProductCode()%>"><%= list.get(i).getProductName() %></a></h3>
                    <span class="price"><%= list.get(i).getSalePrice() %></span>
                </div>
            </div>
        </div>
        <%
